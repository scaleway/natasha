...
void nat_reset_lookup_table(struct nat_address ***nat_lookup);

int add_rules_to_table(struct nat_address ****nat_lookup,
                       uint32_t int_ip, uint32_t ext_ip,
                       unsigned int socket_id);

int nat_dump_rules(int out_fd, struct nat_address ***nat_lookup);
int nat_number_of_rules(struct nat_address ***nat_lookup);
